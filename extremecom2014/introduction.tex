...
Applications in networks in general, and in delay-tolerant networks specifically, often desire to both track the path(s) its data follows through the network and to directly influence the movement of the data through the network. At the same time, in delay-tolerant networks, the networks themselves are almost always integrated into some physical space that also influences that paths of nodes, data, and the phenomena about which the nodes communicate. In traditional IP networks, the {\bf traceroute} tool and {\bf source routing} protocols have accomplished these goals. However, in both traceroute and source routing, the focus in on the logical network hops that the data follows and not on capturing the space-time dynamics of the data's {\em trajectory}.

{\color{red} A couple of quick motivating scenarios: one for tracking, one for routing, and one (the toy maze example) for tying the two together)}

On the tracking side ... {\color{red} details about traceroute}

On the routing side ... {\color{red} details about source routing and geo-routing}

{\color{red} In this context, our novel contributions are...}


{\color{blue}Since the dawn of time, man has wanted to know where the heck his bundles went, and to send them back on a specific geo-coded route.  Our new BreadCrumb Router and its associated extension block processors satisfiy this cosmic desire of human existence.  
