...
Our {\sc breadcrumb} router implements geo-source routing, which allows for delivery paths to be specified in the form of GPS locations. We also introduce {\em GeoRouting} and {\em GeoTracking} blocks that are attached to bundles in order to inform our routing decisions. Our maze experiment, in particular, shows that, by using our router, bundles can successfully follow a series of waypoints without getting lost. We use an incremental, greedy routing approach that we call breadcrumb routing. This allows bundles to continually make progress towards the next waypoint, while not veering off track. The work in this paper lays a foundation for further exploration of trajectory-based geo-source routing.
